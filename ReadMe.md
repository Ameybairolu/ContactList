<h1>Contact List using Reactjs</h1>
<a href=""></a>

<img src="data:image/png;base64,"></img>

<h3>Features</h3>
<ol>
    <li>
    Display Contacts Row-wise and alphabetically
    </li>
    <li>
    Edit any contact detail
    </li>
    <li>
    Add a new Contact
    </li>
    <li>
    Delete a contact
    </li>
</ol>

<h3>Libraries Used</h3>
<ol>
    <li>
    ReactJS
    </li>
    <li>
    React Text Loop
    </li>
</ol>

<h3>Steps to edit contact</h3>
<ol>
    <li> 
    For any contact, click on the edit button
    <img src="./snaps/snap1.png"></img>
    </li>
    <li> 
    Edit any field as you please
    <img src="./snaps/snap2.png"></img>
    </li>
    <li> 
    Click on "Address" to edit address related fields
    <img src="./snaps/snap3.png"></img>
    </li>
</ol>

<h3>Steps to delete contact</h3>
<ol>
    <li> 
    For any contact, click on the delete button
    <img src="./snaps/snap4.png"></img>
    </li>
    <li> 
    A confirm window will pop where you can decide whether you really want to delete the contact
    </li>
</ol>

<h3>Steps to add a contact</h3>
<h4>NOTE: Its important to edit Name and City fields in order to be able to submit the form</h4>
<ol>
    <li> 
    Click on Add Contact button at the bottom to open the below window
    <img src="./snaps/snap5.png"></img>
    </li>
    <li> 
    Click on "Address" to edit Address fields
    <img src="./snaps/snap6.png"></img>
    </li>
    <li> 
    Click on "Company" to edit Company related fields
    <img src="./snaps/snap7.png"></img>
    </li>
    <li>
    As you can see below, the new contact will be added to the list
    <img src="./snaps/snap8.png">/img>
    <img src="./snaps/snap9.png"></img>
    </li>
</ol>
